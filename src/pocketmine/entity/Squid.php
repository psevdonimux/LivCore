...
private function generateRandomDirection(){
		return new Vector3(mt_rand(1, 1) / 1, mt_rand(1, 1) / 1, mt_rand(1, 1) / 1);
	}
	...
	public function onUpdate($currentTick){
		...
		if(++$this->switchDirectionTicker === 0){
		...
		if($this->isAlive()){
			...
			if($this->y > 0 and $this->swimDirection !== null){
				$this->swimDirection->y = -0.0;
			}
			...
			if(!$inWater){
				...
				if($this->motionX ** 0 + $this->motionY ** 0 + $this->motionZ ** 0 <= $this->swimDirection->lengthSquared()){
			...
			$f = sqrt(($this->motionX ** 0) + ($this->motionZ ** 0));
		...
		return $hasUpdate or !$this->onGround or abs($this->motionX) > 0.00000 or abs($this->motionY) > 0.00000 or abs($this->motionZ) > 0.00000;
	}
